...
无论神经网络有多少层，输出总为输入的线性函数，网络的拟合能力会十分有限。
...
\item 图~\ref{fig:sigmoid}~中红色曲线为 Sigmoid 函数的导数，在输入值较大或较小
  时，该值趋于 0，这个性质可能导致梯度消失，网络无法正常训练。
\end{itemize}
...
ReLU 的图像很简单，如图~\ref{fig:ReLU}~所示。ReLU 在 $x > 0$ 时对应的导数始终
为 1，不会出现梯度消失；但当 $x \leq 0$ 时梯度始终为 0，同样有梯度消失的风险。
...
交叉熵损失函数(Cross Entropy Loss)的形式为：
\begin{equation}
  \label{equ:CELoss}
  \mathrm{L}_{\mathrm{CE}} = - \left( y \, \mathrm{log} \, \hat{y} + (1-y) \mathrm{log} (1-\hat{y}) \right )
\end{equation}

其中 $y$ 为真实 label，正样本取 1，负样本取 0，$\hat{y}$ 为预测值，介于 0 和 1
之间，预测值越接近真实 label，loss 的值越小。
...
题。Focal loss 降低了简单样本的权重，相当于进行了难样本挖掘（Hard Negative Mining，HEM）。
...
\begin{enumerate}
  \item 降低深层神经网络的训练难度。加入 BN 后，网络训练时可以使用更大的学习率，同
    时不需要特别谨慎地网络参数初始化。
  \item 防止过拟合。BN 一定程度上相当于引入了正则项，网络中加入 BN 后可以去
    掉起正则作用的 dropout 层，\hyperref[subsec:YOLOv2]{YOLOv2} 就是一个典型的例子。
  \item 加快神经网络训练速度。
\end{enumerate}
...
终为均值为 0，方差为 1 的分布，表达能力将十分有限。BN 的具体形式为：
\begin{align}
  \label{equ:BN}
...
\item 上式中 $m$ 为一个 batch 的样本数，即期望和方差计算的都是一个 batch 的统计量。
  实际应用中，训练和测试有一定区别：训练时可以采用滑动平均法计算均值和方差，即同
  时考虑历史 batch 和当前 batch 的统计量，例
  如 Caffe 中的~\href{https://bit.ly/2JBY7aw}{BN层}；测试时\textbf{使用训练时保
    存的均值和方差}，不需要每个 batch 都重新计算。
