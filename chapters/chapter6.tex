...
过选取不同 scale 的最小 loss，解决人为设定的 anchor 和 scale 对应关系可能并非最
优的问题。
...
在 RetinaNet FPN 结构每个尺度的分类和回归最后一个 feature map 上，分别接对应的平
行分支。

\begin{itemize}
  \item 分类分支：接 $3 \times 3$ 的卷积和 sigmoid，输出尺寸为 $W \times H \times K$，每个点
    取值区间为 [0, 1]，越大表示越可能是物体中心。
  \item 回归分支：接 $3 \times 3$ 的卷积和 ReLU，输出尺寸为 $W \times H \times
    4$，每个点均为正值，表示该点到上/下/左/右四条边的距离。
\end{itemize}
