...
\begin{enumerate}
  \item 取生成的分数最高的 top $N_{\mathrm{pre}}$ 个 proposal。
  \item 给定 IoU 阈值，对第 1 步的所有 proposal 做 NMS。
  \item 取第 2 步 NMS 之后分数最高的 top $N_{\mathrm{post}}$ 个 proposal。
\end{enumerate}

需要强调的是，后处理得到 proposal 后，RPN 的分数就不需要了，proposal 正负样本的
判断与 RPN 分数无关，\textbf{只与其与 GT 的 IoU 有关}。

\section{FPN}
\label{sec:FPN}

FPN 的核心思想是融合不同尺寸的 feature 得到 feature 金字塔，FPN 及不同方法的对比
如图~\ref{fig:FPN}~所示：
