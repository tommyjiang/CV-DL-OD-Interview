...
\paragraph{R-FCN 的损失函数和 RoI 正负样本}
...
\begin{itemize}
  ...
  \item R-FCN：计算量主要集中在卷积得到 PS-RoI Pooling 层需要的 $(C+1+4)k^2$ 个
    feature map。
\end{itemize}

Light-Head R-CNN 的检测 head 综合借鉴了 Faster R-CNN 和 R-FCN，用通道数较少
的 feature + pooling + 一个全连接的轻量 head 提高模型速度。三种模型的结构对比如
